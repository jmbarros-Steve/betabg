...
interface GooglePartnerSetupProps {
  clientId: string;
  onConnected: () => void;
}
...
const LEADSIE_GOOGLE_URL = import.meta.env.VITE_LEADSIE_GOOGLE_REQUEST_URL || '';
...
export function GooglePartnerSetup({ clientId, onConnected }: GooglePartnerSetupProps) {
  ...
  const pollForConnection = useCallback(async () => {
    const { data } = await supabase
      ...
      .select('id, is_active, account_id, connection_type')
  ...
  if (state === 'connected') {
    return (
      <div className="text-center py-8 space-y-4">
        ...
        <div>
          <h3 className="text-lg font-semibold text-emerald-900">Google Ads conectado</h3>
          <p className="text-sm text-muted-foreground mt-1">
            Tu cuenta de Google Ads se conecto correctamente. Ya puedes ver tus metricas.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-6 py-4">
      <div className="text-center space-y-2">
        <h3 className="text-lg font-semibold">Conecta tu cuenta de Google Ads</h3>
        <p className="text-sm text-muted-foreground">
          Haz clic en el boton para compartir acceso a tu cuenta de Google Ads con Steve.
      ...
      <div className="bg-muted/50 rounded-lg p-4 border space-y-3">
        ...
        <ol className="text-sm text-muted-foreground space-y-2 list-decimal list-inside">
          <li>Haz clic en <span className="font-medium text-foreground">"Conectar Google Ads"</span></li>
          <li>Inicia sesion con tu cuenta de Google</li>
          <li>Acepta compartir acceso a tu cuenta publicitaria</li>
      ...
      <div className="flex items-center justify-center gap-2">
        ...
        <span className="text-xs text-muted-foreground">
          Conexion segura via Google MCC
        </span>
      </div>
    </div>
  );
