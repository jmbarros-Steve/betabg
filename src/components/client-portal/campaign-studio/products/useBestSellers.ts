...
export function useBestSellers(clientId: string, options?: UseBestSellersOptions) {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      const { data, error: fnError } = await supabase.functions.invoke('fetch-klaviyo-top-products', {
        body: { connectionId: conn.id, metric: 'ordered', timeframe, limit },
      });

      if (fnError || !data?.products) {
        setError(fnError?.message || 'Error al obtener productos más vendidos');
        setProducts([]);
        return;
      }
