...
import GrapesEmailEditor, { type UnlayerEditorRef } from '../../email/GrapesEmailEditor';
...
interface GrapesStudioEmailEditorProps {
  emails: EditorEmail[];
...
export function GrapesStudioEmailEditor({
  emails: initialEmails,
  onSave,
  onCancel,
  clientId,
  brandColor,
}: GrapesStudioEmailEditorProps) {
  const editorRef = useRef<UnlayerEditorRef>(null);
  ...
  const handleEditorReady = useCallback(() => {
    ...
    if (email) {
      if (email.designJson) {
        editorRef.current?.loadDesign(email.designJson);
      } else if (email.htmlContent) {
        editorRef.current?.setHtml(email.htmlContent);
      }
    }
  }, [initialEmails]);

  const saveCurrentEmail = useCallback(async (): Promise<EditorEmail[]> => {
  ...
  const handleTabChange = useCallback(
    async (newIndex: number) => {
      ...
      if (nextEmail) {
        if (nextEmail.designJson) {
          editorRef.current?.loadDesign(nextEmail.designJson);
        } else if (nextEmail.htmlContent) {
          editorRef.current?.setHtml(nextEmail.htmlContent);
        }
      }
    },
