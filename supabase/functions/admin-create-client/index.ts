  ...
  try {
    ...
    if (action === 'setup_patricio') {
      ...
      if (clientErr) {
        ...
        return new Response(JSON.stringify({ ...results, error: clientErr.message, step: 'create_client' }), {
    ...
    if (action === 'diagnostic') {
      ...
      return new Response(JSON.stringify({ clients: allClients, connections: allConns, roles: allRoles }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
