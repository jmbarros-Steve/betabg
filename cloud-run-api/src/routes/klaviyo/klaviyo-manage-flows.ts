...
import {
  ...
  klaviyoGet, klaviyoPost, klaviyoPatch,
  fetchLogoFromKlaviyo, fetchClientBrand, deleteKlaviyoTemplate,
  escapeHtml,
  type BrandData,
} from './_helpers.js';

const KLAVIYO_FLOW_REVISION = KLAVIYO_REVISION;
...
export async function klaviyoManageFlows(c: Context) {
  try {
    ...
    switch (action) {
      ...
      case 'activate_flow':
        return await handleActivateFlow(c, apiKey, body);
      default:
...
function getShopifyProductBlock(flowType: string, brandName: string): string {
  if (flowType === 'abandoned_cart') {
    return `
        <!-- PRODUCTOS DEL CARRITO (dinamico desde Shopify) -->
        <tr>
            ...
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e8e8e8;border-radius:8px;overflow:hidden;">
              <tr>
                <td style="background:#fafafa;padding:12px 16px;border-bottom:1px solid #e8e8e8;">
                  <p style="margin:0;font-size:13px;font-weight:600;color:#666;text-transform:uppercase;letter-spacing:1px;">Tu carrito en ${escapeHtml(brandName)}</p>
                </td>
              </tr>
  ...
  return `
        <!-- PRODUCTOS RECOMENDADOS (dinamico desde Shopify) -->
        <tr>
          <td class="mobile-padding" style="padding:8px 40px 24px;">
...
// ===============================================================
// Generic email content per flow type — dynamic, brand-agnostic
// ===============================================================
    ...
    const steps = [
      {
        ...
        body: `<p style="margin:0 0 16px;font-size:16px;line-height:1.7;color:#333;">Hace tiempo que no nos visitas y queríamos saber cómo estás. En <strong>${escapeHtml(brandName)}</strong> valoramos a cada miembro de nuestra comunidad.</p>
<p style="margin:0 0 16px;font-size:16px;line-height:1.7;color:#333;">Desde tu última visita hemos renovado nuestro catálogo con nuevos productos y marcas que te van a encantar. ¡Ven a descubrirlos!</p>`,
        ctaText: 'Ver novedades',
      },
      {
        heading: 'Novedades que no te puedes perder',
        body: `<p style="margin:0 0 16px;font-size:16px;line-height:1.7;color:#333;">En <strong>${escapeHtml(brandName)}</strong> siempre estamos buscando lo mejor para ti. Revisa nuestros productos más recientes y encuentra exactamente lo que necesitas.</p>`,
        ctaText: 'Explorar productos',
      },
      {
        heading: '¡Último sprint! Un regalo para ti',
        body: `<p style="margin:0 0 16px;font-size:16px;line-height:1.7;color:#333;">Porque valoramos a cada miembro de nuestra comunidad, queremos darte un incentivo especial para que vuelvas.</p>`,
    ...
    const steps = [
      {
        ...
        body: `<p style="margin:0 0 16px;font-size:16px;line-height:1.7;color:#333;">Vimos que dejaste algo increíble en tu carrito y queríamos asegurarnos de que no se te escape. Tu selección te espera exactamente como la dejaste.</p>
<p style="margin:0 0 16px;font-size:16px;line-height:1.7;color:#333;">Los productos que elegiste son de <strong>alta calidad</strong> y están entre los más populares de <strong>${escapeHtml(brandName)}</strong>.</p>`,
        ctaText: 'Volver a mi carrito',
      },
      {
        heading: '¿Tenías dudas? Estamos acá para ayudarte',
        body: `<p style="margin:0 0 16px;font-size:16px;line-height:1.7;color:#333;">Entendemos que a veces hay que pensarlo. Tu carrito sigue guardado con los productos que seleccionaste. Solo falta un click para que sean tuyos.</p>
<p style="margin:0 0 16px;font-size:16px;line-height:1.7;color:#333;">Si tienes preguntas sobre tallas, compatibilidad o cualquier detalle, responde este correo y te ayudamos sin compromiso.</p>`,
        ctaText: 'Completar mi compra',
      },
      {
        heading: 'Última oportunidad para tu selección',
        body: `<p style="margin:0 0 16px;font-size:16px;line-height:1.7;color:#333;">Tu carrito sigue esperándote, pero no podemos garantizar stock ni precios por mucho más tiempo. <strong>Este es tu último recordatorio.</strong></p>`,
  ...
  if (flowType === 'welcome_series') {
    const steps = [
      {
        heading: `¡Bienvenido/a a ${escapeHtml(brandName)}!`,
        body: `<p style="margin:0 0 16px;font-size:16px;line-height:1.7;color:#333;">¡Nos alegra mucho que te unas a nuestra comunidad! Somos mucho más que una tienda — somos un equipo apasionado que selecciona los mejores productos para ti.</p>
<p style="margin:0 0 16px;font-size:16px;line-height:1.7;color:#333;">Explora nuestro catálogo y descubre por qué miles de clientes confían en nosotros.</p>`,
        ctaText: 'Explorar la tienda',
      },
      {
        heading: 'Encuentra tu producto perfecto',
        body: `<p style="margin:0 0 16px;font-size:16px;line-height:1.7;color:#333;">En <strong>${escapeHtml(brandName)}</strong> queremos ayudarte a encontrar exactamente lo que necesitas. Revisa nuestras categorías más populares y descubre nuestros productos destacados.</p>`,
        ctaText: 'Ver categorías',
      },
      {
        heading: '¡Un regalo de bienvenida!',
        body: `<p style="margin:0 0 16px;font-size:16px;line-height:1.7;color:#333;">Por ser parte de nuestra comunidad, queremos regalarte algo especial para tu primera compra. Es nuestra forma de decirte: bienvenido/a, estás en el lugar correcto.</p>`,
  ...
  // Default / campaign
  return {
    heading: `${name} ${escapeHtml(brandName)} tiene algo para ti`,
    body: `<p style="margin:0 0 16px;font-size:16px;line-height:1.7;color:#333;">Descubre lo último que tenemos preparado para ti. En <strong>${escapeHtml(brandName)}</strong> trabajamos para traerte los mejores productos del mercado.</p>`,
    ctaText: 'Ver más',
  };
}
...
// ===============================================================
// Build flow definition for Klaviyo API
// ===============================================================
...
// ===============================================================
// Action: create_flow
// Creates templates + actual flow in Klaviyo
// ===============================================================
async function handleCreateFlow(
  ...
  // === STEP 2: Try to create actual flow ===
  ...
  try {
    ...
    console.log(`[create_flow] Attempting flow creation with ${definition.actions.length} actions...`);
    ...
    flowError = e.message;
    console.log(`[create_flow] Flow creation failed (templates still created): ${e.message}`);
    // Cleanup: delete orphaned templates if flow creation failed
    console.log(`[create_flow] Cleaning up ${templateIds.length} orphaned templates...`);
    await Promise.all(templateIds.map(id => deleteKlaviyoTemplate(apiKey, id)));
  }
  ...
  const { data: plan, error: planError } = await serviceClient
    ...
    .insert({
      ...
      emails: emailSteps,
      status: 'draft',
      admin_notes: `${flowCreated ? 'FLOW CREATED' : 'FLOW CREATION FAILED'} on ${new Date().toISOString()}. ${templateIds.length} templates. Template IDs: ${templateIds.join(', ')}. ${flowId ? `Klaviyo Flow ID: ${flowId}.` : ''} Trigger type: ${triggerType || 'manual'}. Connection: ${connection.id}.${flowError ? ` Error: ${flowError}` : ''}`,
    })
    .select()
    .single();
  ...
  return c.json({
    ...
    message: flowCreated
      ...
      : `No se pudo crear el flow en Klaviyo. ${flowError || 'Error desconocido'}. Los templates fueron limpiados.`,
    plan_id: plan?.id || null,
    templateIds: flowCreated ? templateIds : [],
  });
}
...
// ===============================================================
// Activate flow (set to live)
// ===============================================================
async function handleActivateFlow(c: Context, apiKey: string, body: any) {
  const { flowId } = body;
  ...
  try {
    const result = await klaviyoPatch(`${KLAVIYO_BASE}/flows/${flowId}/`, apiKey, {
      data: {
        type: 'flow',
        id: flowId,
        attributes: {
    ...
    const newStatus = result?.data?.attributes?.status || 'live';
    console.log(`[activate_flow] Flow ${flowId} activated → ${newStatus}`);
    ...
    console.error('[activate_flow] Error:', e.message);
    return c.json({ error: `Error al activar flow: ${e.message}` }, 500);
  }
}
...
<body style="margin:0;padding:0;background-color:#f4f4f4;font-family:Arial,'Helvetica Neue',Helvetica,sans-serif;-webkit-text-size-adjust:100%;-ms-text-size-adjust:100%;">
  ${config.previewText ? `<div style="display:none;max-height:0;overflow:hidden;mso-hide:all;font-size:1px;color:#f4f4f4;line-height:1px;">${escapeHtml(config.previewText)}&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;</div>` : ''}
      ...
      <table role="presentation" class="email-container" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background-color:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
        ...
        <tr>
          <td style="padding:28px 40px 12px;text-align:center;">
            <p style="margin:0 0 6px;font-size:15px;color:#333;line-height:1.5;">¡Te esperamos!</p>
            <p style="margin:0 0 4px;font-size:15px;color:#333;">Un abrazo,</p>
            <p style="margin:0;font-size:15px;font-weight:700;color:#1a1a1a;">El equipo de ${escapeHtml(brand.name)}</p>
          </td>
        </tr>
