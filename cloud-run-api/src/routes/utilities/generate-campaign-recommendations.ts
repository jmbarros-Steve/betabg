  ...
  if (ANTHROPIC_API_KEY && campaigns.length > 0) {
    try {
      ...
      const [
        ...
        supabase.from('steve_knowledge').select('categoria, titulo, contenido').in('categoria', [platformCategoria, 'anuncios']).eq('activo', true).order('orden', { ascending: true }).limit(8),
